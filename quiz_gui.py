...
def resource_path(relative_path):
	...
	# PyInstaller が作成したテンポ-リフォルダが設定されている場合はそちらを使い、
...
class QuizManager:
	...
	def __init__(self, manager, screen_rect, required_answers, country_data):
		...
		# --- ▼▼▼ 追加 ▼▼▼ ---
		self.clear_sound = None         # 再生中のクリアサウンド
		self.clear_message_window = None  # 表示中のクリアウィンドウ
		# --- ▲▲▲ 追加 ▲▲▲ ---
	...
	def create_window(self):
		...
		if not self.answered_countries:
			html_content = title 
		else:
			# リストに中身がある場合のみ、インデント処理を実行
			html_content = f"{title}{self.answered_countries[0]}"

			# 2行目以降 
			if len(self.answered_countries) > 1:
				# インデント (全角スペース4つ分)
				indent = "&emsp;&emsp;&emsp;&emsp;"
				
				# 2行目以降の国名リストを作成
				remaining_countries_html = [f"{indent}{country}" for country in self.answered_countries[1:]]
				
				# <br> (HTMLの改行) で連結
				html_content += "<br>" + "<br>".join(remaining_countries_html)

		self.answered_label = pygame_gui.elements.UITextBox(
			# text 引数を html_text 引数に変更し、<br>タグが解釈されるようにする
			html_text=html_content, 
			relative_rect=pygame.Rect(10, 180, 460, 100),
			manager=self.manager, 
			container=self.quiz_window,
			#IDを変更し、テーマ(json)側で @quiz_textbox としてスタイル指定できるようにする
			object_id=pygame_gui.core.ObjectID(class_id='@quiz_textbox', object_id='#answered_countries_textbox')
		)
	...
	def process_event(self, event):
		...
		if event.type == pygame_gui.UI_WINDOW_CLOSE:
			...
			# --- ▼▼▼ 追加（クリアウィンドウが閉じられたかチェック） ▼▼▼ ---
			if self.clear_message_window and event.ui_element == self.clear_message_window:
				#print("    -> This was the CLEAR Window. Stopping sound.")
				if self.clear_sound:
					self.clear_sound.fadeout(500) # 0.5秒かけてフェードアウト
					self.clear_sound = None
				self.clear_message_window = None # 参照をクリア
				return
			# --- ▲▲▲ 追加 ▲▲▲ ---
	...
	def check_answer(self):
		...
		if correct_country_name and correct_country_name not in self.answered_countries:
			...
			if len(self.answered_countries) >= self.required_answers:
				...
				try:
					sound_path = resource_path('clear.mp3') 
					
					# 既存のサウンドが再生中なら停止
					if self.clear_sound:
						self.clear_sound.stop()

					self.clear_sound = pygame.mixer.Sound(sound_path)
					self.clear_sound.play(loops=-1) # <-- loops=-1 で無限ループ再生
				
				except Exception as e:
					print(f"ERROR: クリアサウンドの再生に失敗しました ({sound_path}): {e}", file=sys.stderr)
	...
	def update_labels(self):
		...
		if self.answered_label:
			...
			# self.answered_countries が空の場合の処理を変更
			if not self.answered_countries:
				html_content = title # <-- 「(まだありません)」を削除
			else:
				# リストに中身がある場合のみ、インデント処理を実行
				first_country = self.answered_countries[0]
				html_content = f"{title}{first_country}"

				# 2行目以降 
				if len(self.answered_countries) > 1:
					indent = "&emsp;&emsp;&emsp;&emsp;&emsp;"
	...
	def show_result_message(self, title, message):
		...
		try:
			...
			# --- ▼▼▼ 追加 ▼▼▼ ---
			if is_clear_window:
				self.clear_message_window = message_window
			# --- ▲▲▲ 追加 ▲▲▲ ---
